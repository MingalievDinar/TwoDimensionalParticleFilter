...
//normpdf(X,muX,sigmaX, Y, muY, sigmaY) computes the Multivariate-Gaussian probability function at values X and Y using the
inline double normpdf(double x, double mux, double stdx, double y, double muy, double stdy) {
    return (1.0/(2*M_PI*stdx*stdy)*exp(-0.5*( squared((x-mux)/stdx) + squared((y-muy)/stdy) )));
};
